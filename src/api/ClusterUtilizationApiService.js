  ...
  try {
    ...
    if (nodeName) {
      if (JSON.parse(localStorage.getItem("needHistoricalData"))) {
        ...
        console.log(
          `History Call + ${clusterUtilizationUrl}/multi-level_nodeInfo?from=${startDate}&minutesAgo=${minutesAgo}&clusterName=${clusterName}&nodeName=${nodeName}&userName=${userName}`
        );
      ...
      if (JSON.parse(localStorage.getItem("needHistoricalData"))) {
        console.log(
          `History Call + ${clusterUtilizationUrl}/multi-level_nodeInfo?from=${startDate}&to=${endDate}&clusterName=${clusterName}&userName=${userName}`
        );

        finalUrl = `${clusterUtilizationUrl}/multi-level_nodeInfo?from=${startDate}&to=${endDate}&clusterName=${clusterName}&userName=${userName}`;
      } else {
        console.log(
          `History Call + ${clusterUtilizationUrl}/multi-level_nodeInfo?from=${startDate}&minutesAgo=${minutesAgo}&clusterName=${clusterName}&userName=${userName}`
        );
