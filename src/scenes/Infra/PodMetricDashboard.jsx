...
// };
  ...
  return (
    <Box height="calc(75vh - 30px)" width={chartWidth} padding="5px" border="1px" style={{ transition: "width 0.3s ease-in-out" }}>
      <div style={{ display: "flex", justifyContent: "flex-start" }}>
        ...
        {podData.title ? (
    <p style={{ marginTop: "0px" }} className='chart-title'>{podData.title}</p>
) : (
    <p style={{ marginTop: "0px" }} className='chart-title'>{podData.title}</p>
)}
      ...
      <ReactApexChart
        options={options}
        series={[{ name: "PODCPUMETRIC", data: filteredData, color: "#C40233" }]}
        type="area"
        height={chartHeight}
        width={"100%"}
      />
...
export default PodMetricDashboard;
